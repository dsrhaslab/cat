...
const topPadding = 20;
const threadPadding = 90;
const clockPadding = 30;
...
export default class TraceDrawer {
  ...
  drawSimilarityInfo() {
    ...
    const rect = this.drawing.rect(2000, clockPosition).attr({ id: 'tooltipRect', x: this.drewThreads.length * (threadPadding * 1.25) }).fill('#f9f6f7');
      ...
      if (event instanceof SocketEvent || event instanceof DiskEvent) {
        ...
        if (event.data_similarities) {
          ...
          if (hundredList.length > 0) {
            add.tspan(' ').newLine().dx(20);
            add.tspan('100% similar to events: ').newLine().dx(20);
          ...
          if (ninetyList.length > 0) {
            add.tspan(' ').newLine().dx(20);
            add.tspan('>= 90% similar to events: ').newLine().dx(20);
            add.tspan('- ').newLine().dx(40);
          }
          ...
          if (eightyList.length > 0) {
            add.tspan(' ').newLine().dx(20);
            add.tspan('>= 80% similar to events: ').newLine().dx(20);
            add.tspan('- ').newLine().dx(40);
          }
  ...
  [drawClockEvents](clock) {
    ...
    events.forEach((event) => {
      ...
      const eventIDLabel = eventGroup.plain(`${event.id}`).font({ size: 13 }).fill('#FFFFFF');
      const bb = eventIDLabel.bbox();
      eventIDLabel.attr('x', threadPosition - (bb.width / 2));
      eventIDLabel.attr('y', clockPosition + (eventRadius / 2));
